...
import {
    changeValueTextArea,
} from "../../../../store/REDUX-TOOLKIT/features/chat-page/chatSlice";
...
import {
    ...
    getStatusWS,
    getTextArea
} from "../../../../store/REDUX-TOOLKIT/features/chat-page/chatSelectors";
import {
    sendMessageToServer,
    startListeningNewMessages
} from "../../../../store/REDUX-TOOLKIT/features/chat-page/chatThunks";
        ...
        <>
            <div className={s.wrapper}>
                ...
                <div className={s.messages}
                    ...
                    <div> {checker ? null : usersMessages!.map((e) =>
                        <div
                            key={Math.random()}
                            className={s.userMessage}>
                            {!e.photo ? <Avatar key={e.userId + "ava"} {...getDefaultAvatar(e.userName,40,40,.5)} /> :
                                <img key={e.userId + "imgAva"} src={e.photo} alt="userfoto"/>}
                                <div key={e.userId + "userId"} className={s.messageStyleName}>
                                <div key={e.userId + "name"} className={s.nameUser}>{e.userName}</div>
                                <div key={e.userId + "message"} className={s.messageUser}>{e.messageUser}</div>
                            </div>
                        </div>)}
                    </div>
            ...
            <div className={s.inputMessages}>
                ...
                <Button
                    ...
                    onClick={() => {dispatch(sendMessageToServer(textArea))
                    }}>Send</Button>
            </div>
        </>
    );
